...
import {
  ...
  type StateLike,
  type StateNode,
  type UnsubscribeCurrentChangeListener,
} from '../graph';
import type { ProvenanceGraph } from '../graph/graph-slice';
import type {
  ProduceWrappedStateChangeFunction,
  TrrackActionFunction,
} from '../registry';
import type { ConfigureTrrackOptions } from './trrack-config-opts';
import { TrrackEvents } from './trrack-events';
import type { RecordActionArgs, Trrack } from './types';
...
function getState<State, Event extends string>(
  ...
  const results = applyPatch(checkpointState, deepClone(patches), true, false);
...
export function initializeTrrack<State = any, Event extends string = string>({
  ...
  const metadata = {
    add(metadata: Record<string, unknown>, node: NodeId = graph.current.id) {
      graph.update(
        graph.addMetadata({
          id: node,
          meta: metadata,
    ...
    latestOfType<T = unknown>(type: string, node: NodeId = graph.current.id) {
      return graph.backend.nodes[node].meta[type]?.at(-1) as
        | Metadata<T>
        | undefined;
    },
    allOfType<T = unknown>(type: string, node: NodeId = graph.current.id) {
      return graph.backend.nodes[node].meta[type] as Metadata<T>[] | undefined;
    },
  ...
  const bookmarks = {
    ...
    is(node: NodeId = graph.current.id) {
      return Boolean(metadata.latestOfType<boolean>('bookmark', node)?.val);
    },
  ...
  return {
    ...
    record({
      ...
      const originalState = getState(this.current, this.graph.backend.nodes);
      ...
      for (let i = 0; i < path.length - 1; ++i) {
        ...
        if (isUndo) {
          if (isStateNode(currentNode))
            sideEffectsToApply.push(...currentNode.sideEffects.undo);
        } else {
    ...
    redo(to: 'latest' | 'oldest' = 'latest') {
      ...
      if (current.children.length > 0) {
        return this.to(
          current.children[to === 'oldest' ? 0 : current.children.length - 1],
        );
      } else {
    ...
    exportObject() {
      return JSON.parse(JSON.stringify(graph.backend)) as typeof graph.backend;
    },
